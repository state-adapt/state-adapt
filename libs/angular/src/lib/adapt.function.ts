...
/**
  ...
  ```typescript
  store = adapt('number', 0);
  sub = this.store.state$.subscribe();
  // global state: { number: 0 }

  store = adapt('featureA.number', 0);
  sub = this.store.state$.subscribe();
  // global state: { featureA: { number: 0 } }

  store = adapt('featureA.featureB.number', 0);
  sub = this.store.state$.subscribe();
  ...
  export class MyService {
    ...
    name = adapt(['name', 'John'], this.nameChange$);
  ...
  export class MyService {
    ...
    name = adapt(['name', 'John'], [this.nameChange$, this.nameChange2$]);
  ...
  sources by using the {@link watch} function.
  ...
  export class MyService {
    ...
    name = adapt(['name', 'John', this.nameAdapter], {
      set: this.nameChange$,
