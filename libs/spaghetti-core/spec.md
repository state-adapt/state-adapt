...
- Call-graph propagation exports effects rather than every implementation command. Mutations of callee-owned allocations remain private until the allocation is stored somewhere externally reachable; mutations after that storage point cross the boundary. Returning a fully constructed value alone does not expose its earlier mutations. Parameter-derived effects also cross and are rebound to caller argument provenance before further propagation; shared, captured, class, module, proven-external, and unresolved effects remain exportable.
