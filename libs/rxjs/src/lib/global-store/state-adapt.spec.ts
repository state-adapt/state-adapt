...
describe('adapt', () => {
  ...
  it('should allow undefined', () => {
    const items = adapt([] as number[], {
      adapter: {
        selectors: {
          findAnything: s => s.find(a => false),
        },
      },
    });
    const values = [] as (number | undefined)[];
    items.findAnything$.pipe(take(1)).subscribe(value => {
      values.push(value);
      // @ts-expect-error value should be number | undefined
      const assumeDefined = () => Math.round(value);
      if (value !== undefined) {
        return value * 2;
      }
    });
...
describe('watch', () => {
  it('should not receive any values when store is inactive', () => {
    ...
    const doubleWatch$ = watch(path, adapter).double$;
    const watchValues = [] as number[];

    doubleWatch$.subscribe(v => {
      watchValues.push(v);
    });

    const count = adapt(0, { adapter, path });
    const sub1 = count.state$.subscribe();
    count.set(1);
    count.set(2);
    sub1.unsubscribe();

    expect(watchValues).toEqual([0, 2, 4]);
  });
});
